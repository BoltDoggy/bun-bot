...
- ✅ **M5（P5 全平台分发）已完成**：无 bun 环境也能一键安装使用
  ...
  - 自测: `bun test` **87 用例 / 514 expect 全绿**；`bash scripts/build.sh` 实测产出 `dist/bun-bot-darwin-arm64`（61MB）+ `.sha256`，`shasum -c` 通过，产物 `run` 子命令自举正常（embedded-runtime-ok 1.3.14）
