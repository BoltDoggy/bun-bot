...
| [ARCHITECTURE.md](./ARCHITECTURE.md) | 现状分析（as-is）：随代码演进更新，当前快照基于 M1（P0+P1）+ skills 能力 + AGENT.md 落地后的实际代码 |
...
- ✅ **M1（P0+P1）已完成**：agent 认识自己、能改自己的文件 —— 自修改最小闭环成立
  ...
  - 自测: `bun test` 17 用例全绿（工具层 + 记忆层 + skills 层 + AGENT.md，零外部依赖）
...
- ✅ **AGENT.md 项目级指令（附加能力）已完成**：项目约定有处安放
  - `AGENT.md`（可选）存在时由 `loadProjectContext` 加载进 [项目] 区块最前，优先级高于 README / docs
  - `buildSystemPrompt` [规则] 第 5 条声明其约束力（内容冲突时以 AGENT.md 为准）；不存在时静默跳过，老项目不受影响
...
> 更新时间：2026-08 · 起点 = 模型支持 1M 上下文 · 最新修订 = ARCHITECTURE 快照对齐 M1 + skills + AGENT.md 后代码
