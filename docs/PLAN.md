...
src/budget.ts         上下文 token 预算与超限摘要
...
### P0 · 认知与记忆 —— 让 agent 知道自己是谁

**目标**：启动时 agent 能准确说出"我是谁、项目结构、上次干了什么"。

改动清单：
- [ ] system prompt 增加结构化"自我认知"区块（见 §4）
- [ ] 新增 `AGENT_STATE.json`：决策、踩坑、TODO、上次任务，运行结束时写回
- [ ] 新增 `MEMORY.md`（人类可读版，git 可追踪）
- [ ] 启动钩子：`loadContext()` 自动加载 README + docs 索引 + 记忆

**验收**：连跑两次任务，第二次能引用第一次的决策。

### P1 · 工具集扩充 —— 让 agent 能真正改自己
...
| 工具 | 说明 |
| --- | --- |
| `read_file` | 读工作区文件，默认完整返回（上限 64KB 可配） |
| `write_file` | 写工作区文件（配合 git 快照） |
| `list_dir` | 列目录，支持 `-a` / 深度限制 |
| `run_bash` | 执行 shell 命令（`cwd` 可指定工作区） |

升级 `run_script`：
- [ ] 支持 `cwd` 参数（默认 tmpdir，可指定工作区）
- [ ] 超时可配（`--timeout-ms`，默认 30s → 长任务可放开）
- [ ] 输出上限 4000 → 65536，截断处带偏移信息

**验收**：`bun run index.ts "把 index.ts 顶部的注释改成两行" ` 能真实落盘并 diff 可见。
...
- [ ] git 安全阀：任何 `write_file` / `run_bash` 触及工作区前，`git add -A && git commit` 打快照
...
1. `bun run index.ts --self` 能安全修改自身代码、跑测试、通过或自动回滚。
2. 一次会话完成"新增工具 + 文档 + 测试 + 收尾"全流程，无需人工干预。
3. 重启后能引用上次会话的决策（记忆持久化生效）。
4. 超过 100 轮工具调用的长任务不丢上下文、不爆预算。
