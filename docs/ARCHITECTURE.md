...
基于对 index.ts / src/ / skills/ / tests/ 的实际阅读与统计，更新于 **M1（P0+P1）+ skills 能力落地之后**。
...
| src/ | tools.ts 336 行 / 12.8 KB · context.ts 98 行 / 5.2 KB · memory.ts 180 行 / 5.5 KB · git.ts 42 行 / 1.4 KB |
...
| 自测 | 15 用例 / 54 expect，零外部依赖（`bun test`）；web-search 另有 `self-test.ts --online` 在线实测 |
...
src/context.ts        系统提示词组装：[身份] [能力] [项目] [记忆] [规则] 五区块 + skills 索引
...
6. **长任务无支撑**：没有进度 checkpoint、没有 `--resume` 续跑、没有上下文预算管理（`budget.ts` 未实现），>100 轮容易迷失。
7. **回滚靠手动**：git 快照已自动打，但测试闸门、自动 revert、审计日志属 P3，尚未落地。
8. **`--self` 模式未实现**：还不能"只出 diff 预览 → 确认后应用"，自迭代仍需人在场确认。
