...
基于对 index.ts / src/ / skills/ / tests/ 的实际阅读与统计，更新于 **M1（P0+P1）+ skills 能力 + AGENT.md 项目指令落地之后**。
...
| 项目级指令 | `AGENT.md`（可选）：存在时由 `loadProjectContext` 加载进 [项目] 区块最前，[规则] 第 5 条声明其约束力（优先级高于 README/docs）；不存在时静默跳过 |
...
src/context.ts        系统提示词组装：[身份] [能力] [项目] [记忆] [规则] 五区块 + skills 索引 + AGENT.md 约束声明
src/memory.ts         记忆读写：AGENT_STATE.json / MEMORY.md + AGENT.md 项目指令 + 项目上下文加载
...
| 行为 | 说明 |
| --- | --- |
| 加载时机 | `loadProjectContext()` 读取工作区根 `AGENT.md`（`readAgentDirective()`），存在则置于项目认知最前 |
| 优先级 | 高于 README / docs：它是用户与 agent 之间的项目级契约 |
| 约束声明 | `buildSystemPrompt` 的 [规则] 第 5 条：内容冲突时以 AGENT.md 为准 |
...
2. 加载项目上下文（`loadProjectContext`）→ AGENT.md（如有）+ README + docs + 文件树 + 记忆
...
6. ~~项目约定无处安放~~ → `AGENT.md` 项目级指令：用户与 agent 的契约，加载进 [项目] 最前 + [规则] 声明约束力，接入通用 AGENT.md 工具链
