...
> 但 bun-bot 会**自我进化**：主线（`dev` / `main`）此后给它自己加上了工具注册表、跨会话记忆、任务模式、测试闸门、全平台分发…… 演变成了你现在看到的完整项目（`index.ts` + `src/` + `tests/` + `scripts/` + `.github/`）。
> 另外还有一条 **`research` 分支**：同样基于 `v0.1.0`，但坚持「极简 + 生态对齐」路线继续迭代（约 270 行单文件 + 轻量扩展）—— 拆分 AGENTS.md / BUN_BOT.md 项目约定、对齐 Claude Code 生态的 `.agents/skills` 技能约定、流式输出默认、记忆协议纯提示词落地。它刻意**不引入**本仓库的工程化能力（安全快照 / 测试闸门 / 权限分级 / 全平台分发），只做最少的理念验证 —— 与主线对比，正好是「轻量理念 vs 工程化」两条路线的样板。
> 想复现文章里的「百行版本」，请切到 `v0.1.0`：
>
> ```bash
> git checkout v0.1.0
> # 或直接在线查看：https://github.com/BoltDoggy/bun-bot/tree/v0.1.0
> # 想看极简理念迭代：git checkout research
> ```
...
> bun-bot 启动时会自动加载并优先遵守它（bun-bot 自研细节可放 `BUN_BOT.md`，**仅 bun-bot 自动加载**，其他 agent 无需阅读）。
...
- 🧭 **自我认知 + 项目级指令**：启动时加载 AGENTS.md + BUN_BOT.md（如有）+ README + docs + 文件树 + 记忆，系统提示词含「身份 / 能力 / 项目 / 记忆 / 规则」五区块；项目根目录的 `AGENTS.md`（通用契约，所有 agent 遵守）+ `BUN_BOT.md`（bun-bot 实现细节，仅 bun-bot 生效）是项目级指令，优先级最高
- 🧩 **skills 组合操作库**：多步、有坑、会过时的操作固化成 SKILL.md（仓库内置 `skills/` 索引 + 生态 `skills` 约定 `.agents/skills/` frontmatter 自描述，双目录兼容，P6-3），按需加载、自带自测
...
| skill | 一句话 | 自测 |
| （生态）`.agents/skills/<name>/SKILL.md` | Claude Code 生态技能：YAML frontmatter 自描述，自动扫描进 [能力] 区块（P6-3） | 见各 SKILL.md |
| --- | --- | --- |
...
| `skillsDir` | `[".agents/skills", "skills"]` | 技能目录（P6-3：支持字符串/数组；.agents/skills 生态约定 frontmatter 自描述 + skills 内置索引，双目录兼容） |
...
> 完整契约见项目根目录的 [AGENTS.md](./AGENTS.md)（通用项目契约，所有 agent 遵守）与 [BUN_BOT.md](./BUN_BOT.md)（bun-bot 实现细节：运行 / 构建 / 测试闸门 / 代码约定 / 架构决策 / 踩坑，**仅 bun-bot 自动加载**，其他 agent 无需阅读），优先级最高，以下为速览。
...
├── BUN_BOT.md            # 可选：项目级指令·bun-bot 实现细节（仅 bun-bot 自动加载，其他 agent 无需阅读，P6-2）
├── .github/
│   └── workflows/
...
├── tests/              # 104 用例 / 598 expect（M1 + skills + AGENTS.md + P2 + P3 + P4 + P5 + P6 全量闸门）
...
- 改完必须跑 `bun test`：**104 用例 / 598 expect**，零外部依赖 —— 任何代码改动后必须全绿（P4 新增 10 个测试文件 + P5 新增 1 个 p5-release + P6 新增 3 个：p6-stream / p6-skills / p6-memory）
