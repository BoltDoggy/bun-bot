...
- 🧭 **自我认知**：启动时加载 AGENT.md（如有）+ README + docs + 文件树 + 记忆，系统提示词含「身份 / 能力 / 项目 / 记忆 / 规则」五区块
- 📜 **AGENT.md 项目级指令**：项目根目录的 `AGENT.md`（可选）是用户与 agent 的项目级契约，存在时自动加载进 [项目] 区块最前、优先级最高（高于 README / docs），[规则] 中声明其约束力；类似 CLAUDE.md 的通用约定，方便接入任何支持 AGENT.md 的 agent 工具链
...
> 可选：在项目根目录放一个 `AGENT.md` 写入项目约定（如禁止改哪些文件、必须跑什么测试），
...
├── AGENT.md            # 可选：项目级指令（存在时自动加载，优先级最高）
├── src/
│   ├── tools.ts        # 工具注册表（新增工具在此注册）
│   ├── context.ts      # 系统提示词组装：身份 + 能力 + 项目 + 记忆 + 规则
│   ├── memory.ts       # 记忆读写：AGENT_STATE.json / MEMORY.md + AGENT.md + 项目上下文
...
```bash
bun test   # 17 个用例：工具层 + 记忆层 + skills 层 + AGENT.md，零外部依赖
